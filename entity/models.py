...
class PLMsForEntity(nn.Module):
    def __init__(self,  args, num_ner_labels, head_hidden_dim=150, width_embedding_dim=150, max_span_length=8):
        ...
        self.args = args
        self.PLM = PLMs[args.model]['model'].from_pretrained(args.model, output_hidden_states=False,
                                                                               return_dict=False)
    ...
    def forward(self, input_ids, spans, spans_mask, spans_ner_label=None, token_type_ids=None, attention_mask=None, epoch=None):
        ...
        if spans_ner_label is not None:
            ...
            if attention_mask is not None:
                ...
                hidden_train = hidden.view(-1, hidden.shape[-1])

                # postive hidden
                legal_index_pos = torch.nonzero((active_labels > 0)*1)
...
class EntityModel():
    ...
    def run_batch(self, samples_list, epoch=None, try_cuda=True, training=True):
